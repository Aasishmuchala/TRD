...
class FIIAgent(BaseAgent):
    ...
    async def analyze(
        ...
        # Conviction from winning-direction samples only (no consistency penalty).
        # The LLM already encodes uncertainty in its raw score — multiplying by
        # consistency double-penalises disagreement and tanks real-LLM scores to
        # ~33, below the 55 conviction floor. Use average conviction of samples
        # that agree with the winning direction instead.
