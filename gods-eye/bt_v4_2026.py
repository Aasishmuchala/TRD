"""
bt_v4_2026.py — God's Eye: 2026 Year-to-Date Validation (Jan 2026 – Apr 4, 2026)
==================================================================================
Runs 4 months of live-model backtest on 2026 data to see how God's Eye performs
on current market conditions (not cherry-picked losing months).

Run from gods-eye/backend:
    cd gods-eye/backend
    set -a && source .env && set +a
    python3 -u ../bt_v4_2026.py 2>&1 | tee ../bt_v4_2026.log
"""
...
# Jan 2026 – Apr 4 2026. Data goes to 2026-04-02 (last trading day).
# Baseline = 0 (no prior system to compare against — just measuring raw P&L)
MONTHS_2026 = [
    ("2026-01-01", "2026-01-31", "Jan-2026", 0),
    ("2026-02-01", "2026-02-28", "Feb-2026", 0),
    ("2026-03-01", "2026-03-31", "Mar-2026", 0),
    ("2026-04-01", "2026-04-04", "Apr-2026", 0),   # only 2 trading days in DB (Apr 1-2)
]
...
async def run_month(engine, from_date, to_date, label, baseline_inr, vix_map, all_ohlcv):
    ...
    for row in signal_rows:
        ...
        if full_idx + 1 >= len(all_ohlcv):
            # Last row in DB — no next day to compute P&L against, skip
    ...
    return {
        ...
        "days": len(signal_rows), "baseline": baseline_inr,
    }
...
async def main():
    ...
    print("=" * 100)
    print("  God's Eye — 2026 YTD Backtest (Jan 2026 → Apr 4 2026)")
    print("  VIX gate + event blackout active  |  Real LLM calls via OpusCode Pro")
    print("=" * 100)
    ...
    # Show 2026 data range
    rows_2026 = [r for r in all_ohlcv if r["date"] >= "2026-01-01"]
    print(f"  2026 data: {len(rows_2026)} trading days  "
          f"({rows_2026[0]['date'] if rows_2026 else 'none'} → "
          f"{rows_2026[-1]['date'] if rows_2026 else 'none'})", flush=True)

    engine = BacktestEngine()
    summaries = []

    for from_date, to_date, label, baseline in MONTHS_2026:
        try:
    ...
    if summaries:
        ...
        print(f"\n{'='*100}")
        print("  FINAL — God's Eye 2026 YTD Performance")
        print(f"{'='*100}")
        ...
        for s in summaries:
            ...
            print(f"  {s['label']:<12}  {fmt_pnl(s['inr']):>10}  {fmt_ret(s['ret_pct']):>8}  "
                  f"{s['trades']:>7}  {wr:>5.0f}%  {sel:>5.0f}%  {st}")
        print(f"  {'-'*70}")
        print(f"  {'YTD TOTAL':<12}  {fmt_pnl(new_total):>10}  {fmt_ret(new_ret):>8}  "
              f"{total_trades:>7}  {overall_wr:>5.0f}%  {overall_sel:>5.0f}%  "
              f"{'✓ PROFITABLE' if new_total > 0 else '✗ NET LOSS'}")
        print(f"\n  Capital: ₹{CAPITAL:,.0f}   YTD return: {fmt_ret(new_ret)}")
        print(f"  Period:  Jan 2026 → Apr 4 2026  ({total_days} trading days, {total_trades} trades taken)")
        print("=" * 100)
        print(flush=True)
